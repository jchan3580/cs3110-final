  ...
  if (String.length str >= 4 &&
    ...
    (let command = String.sub (String.lowercase str) 4
      ...
      String.sub (String.lowercase str) 0 5 = "move ") then
        (room:=(gen_room_items initial_item_list initial_pokeML_list);())
...
let player1 = {hunger= ref 100;
  thirst= ref 100;
  xp= ref 0;
  level= ref 1;
  inventory= initial_item_list;
  pokeML=initial_pokeML_list;
  } in

print_string "Welcome to pokeML!";
  main player1 (ref (gen_room_items initial_item_list initial_pokeML_list))

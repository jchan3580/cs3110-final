...
module Draw = struct
	...
	let get_tga_pixel s x y w bitsperpixel =
		let bpp = bitsperpixel / 8 in
		let b = int_of_char (Bytes.get s (0 + (y*w + x) * bpp))
		and g = int_of_char (Bytes.get s (1 + (y*w + x) * bpp))
		and r = if bitsperpixel > 16 then int_of_char (Bytes.get s (2 + (y*w + x) * bpp)) else 0
		and a = if bitsperpixel > 24 then int_of_char (Bytes.get s (3 + (y*w + x) * bpp)) else 1
		in
